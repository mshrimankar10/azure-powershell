...
try{
    ...
    $job = start-job {
        ...
        Update-MarkdownCommandHelp -Path "$helpPath\*-*.md"
        $updatedHelp = Import-MarkdownCommandHelp -Path "$helpPath\*-*.md"
        $moduleFile = Get-ChildItem -Path $helpPath -Filter "Az.$ModuleRootName.md" | Select-Object -First 1
        if ($moduleFile) { Update-MarkdownModuleFile -Path $moduleFile.FullName -CommandHelp $updatedHelp }
